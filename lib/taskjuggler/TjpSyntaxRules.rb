...
EOT
  ...
  def rule_accountAttributes
    ...
    pattern(%w( _supplement !supplementAccount !accountBody ), lambda {
      @property = @idStack.pop
    })
    doc('supplement.account', <<'EOT'
The supplement keyword provides a mechanism to add more attributes to already
defined accounts. The additional attributes must obey the same rules as in
regular account definitions and must be enclosed by curly braces.

This construct is primarily meant for situations where the information about a
account is split over several files.
EOT
       )
    also('supplement')
  end
...
EOT
    ...
    doc('supplement', <<'EOT'
The supplement keyword provides a mechanism to add more attributes to already
defined accounts, report, resources or tasks. The additional attributes must
obey the same rules as in regular definitions and must be enclosed by curly
braces.
...
EOT
    ...
    also(%w( supplement.account supplement.report supplement.resource supplement.task ))
    example('Supplement')
...
EOT
    ...
    pattern(%w( _supplement !supplementReport !reportBody ), lambda {
      @property = @idStack.pop
    })
    doc('supplement.report', <<'EOT'
The supplement keyword provides a mechanism to add more attributes to already
defined reports. The additional attributes must obey the same rules as in
regular report definitions and must be enclosed by curly braces.

This construct is primarily meant for situations where the information about a
report is split over several files.
EOT
       )
    also('supplement')
...
EOT
  ...
  def rule_resourceAttributes
    ...
    pattern(%w( _supplement !supplementResource !resourceBody ), lambda {
      @property = @idStack.pop
    })
